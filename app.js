...
function setActiveView(view) {
    ...
    document.querySelectorAll('.menu-item[data-view]').forEach(item => item.classList.toggle('active', item.dataset.view === view));
    if (window.innerWidth <= 768) document.getElementById('main-nav').classList.remove('open');
}
function ui(key, fallback = '') { return wiki.ui[key] || fallback; }
...
function renderTravelLevel(travel = {}) {
    return ['Land', 'Forest', 'Mountain', 'Sea', 'Hot', 'Cold']
...
function characterSearchText(character) {
    ...
    return [key, characterText(key, 'Name'), character.Gender, character.Faction, character.Role, character.TraitKey1, character.TraitKey2, character.TraitKey3, characterText(key, 'Time'), characterText(key, 'Background')].filter(Boolean).join(' ').toLowerCase();
}
function renderCharacters() {
    const cards = wiki.characters.map(character => {
        ...
        return `<article class="card character-card entity-list-card" data-key="${escapeHtml(key)}" data-search="${escapeHtml(characterSearchText(character))}" onclick="loadCharacterDetail(this.dataset.key)">${renderCharacterImage(character, name)}<div class="info-list list-info">${previewInfo}</div></article>`;
    }).join('');
...
function jobSearchText(job) {
    return [job.JobKey, jobText(job.JobKey), job.Role, job.PassiveSkillKey, job.ActiveSkillKey, job.BoostType, ...(job.PrerequisiteJobs || [])].filter(Boolean).join(' ').toLowerCase();
}
function renderJobs() {
    const cards = wiki.jobs.map(job => {
        const name = jobText(job.JobKey);
        const previewInfo = [infoRow(ui('job', 'Job'), escapeHtml(name), 'entity-primary-row'), infoRow(ui('rarity', 'Rarity'), escapeHtml(rarityStars(job.Rarity))), infoRow(ui('role', 'Role'), escapeHtml(job.Role || ''))].join('');
        return `<article class="card job-card entity-list-card" data-key="${escapeHtml(job.JobKey)}" data-search="${escapeHtml(jobSearchText(job))}" onclick="loadJobDetail(this.dataset.key)">${renderJobImage(job, name)}<div class="info-list list-info">${previewInfo}</div></article>`;
    }).join('');
    return `<div class="header-card"><h1>${escapeHtml(ui('jobs', 'Jobs'))}</h1><p><strong>${wiki.jobs.length} ${escapeHtml(ui('entries', 'entries'))}</strong></p><input type="text" id="searchInput" class="search-input" placeholder="${escapeHtml(ui('search', 'Search...'))}" aria-label="${escapeHtml(ui('search', 'Search...'))}"></div><div class="grid" id="jobGrid">${cards}</div>`;
}
...
function loadView(view, fromRoute = false) {
    ...
    else if (view === 'Jobs') { content.innerHTML = renderJobs(); attachListSearch('.job-card'); }
...
function createWarpBurst(x, y) {
    ...
    const ringInner = document.createElement('span'); ringInner.className = 'warp-ring warp-ring-inner';
    const ringOuter = document.createElement('span'); ringOuter.className = 'warp-ring warp-ring-outer';
...
async function loadJson(path, fallback) {
    try { const response = await fetch(path); return response.ok ? await response.json() : fallback; }
    catch (error) { console.error(`Failed to load ${path}`, error); return fallback; }
}
async function init() {
